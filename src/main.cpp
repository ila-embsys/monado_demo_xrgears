...
public:
  ...
  init_quads()
  {
    ...
    xr_quad_init(&xr.quad, xr.session, xr.local_space,
                 extent, pose, size);
    ...
    for (uint32_t i = 0; i < xr.quad.swapchain_length; i++)
    {
        ...
        quad_texture[i].load_from_ktx(xr.quad.images[i].image, hawk_ktx,
                                      tex_size, vk_device, queue);
    ...
    xr_quad_init(&xr.quad2, xr.session, xr.local_space,
                 extent2, pose2, size2);
